...
The purpose of this experience is to demonstrate that the fractal dimension
of our aluminum tinfoil spheres , neither completely full nor completely empty, is
between 2 and 3. Following the procedure shown in (\ref{subsec:Experimental procedure}),
we measured the sides of the alluminum squares and then crumpled in to alluminum spheres.
All the collected data in shown in the tables (\ref{tab:squares}), (\ref{tab:balls}). 
Next, we analyzed the graphs (\ref{fig:mass_vs_length}), (\ref{fig:ln_mass_vs_ln_length}), (\ref{fig:mass_vs_diameter}), (\ref{fig:ln_mass_vs_ln_diameter}) by initially applying a power law, followed by linear 
regression after linearizing the equation (\ref{eq:gen_fractal}).
After that, is it possible to compare te results obtained with the power law with the ones obtained
