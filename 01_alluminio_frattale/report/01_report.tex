...
\par Fractal objects are characterised by the property of self-similarity; 
in simpler words, they look the same when observed at different scales.
One of the most famous fractal object is the Mandelbrot set, but we can find 
other examples also in nature, such as in the structure of a Romanesco Cauliflower, or, 
...
lenghts of the square as represented in Fig (\ref{fig:sq_measure}). Finally, we collect all data in an excel spreadsheet which we will show in the next section.
\begin{figure}[h]
    \centering
...
Once finished, our last measurent is to collect the information of the linear dimension 
