...
Once finished, our last measurent will be to collect the information of the linear dimension (its diameter) 
...
As expected, we obtained that the data is well represented by a linear function, whose parameters $\ln(k)$ and $\alpha$ correspond to the slope and the intercept of the line, respectively. The values obtained for these parameters are $\ln(k) = -6.07 \pm (...)$ and $\alpha = 2.13 \pm (...)$. These values are in perfect agreement with the ones obtained from the previous method, which validates the results obtained from the experiment.
% Conclusion
\section{Conclusion}
